...
export async function main(ns: NS) {
  ...
  const portData = ns.getPortHandle(port);
  ...
  try {
    ...
    while (true) {
      ...
      if (!flags.only) await processTargets(ns, targets, maxHackMoney, baseDelay, portData);
      ...
      if (flags.only) {
        ...
        await processSingleTarget(ns, onlyTarget, maxHackMoney, baseDelay, portData);
      }
...
// Xử lý nhiều server
async function processTargets(ns: NS, targets: string[], maxHackMoney: number, baseDelay: number, portData: any) {
  for (const target of targets) {
    ...
    // 1. CHUẨN HÓA
    const normalized = await normalizeTarget(ns, target, scripts, ram, hosts, {portData: portData});
    if (normalized) {
    ...
    HackUtils.updateLog(ns, {portData: portData}, target, batch);
...
// Xử lý 1 target
async function processSingleTarget(ns: NS, target: string, maxHackMoney: number, baseDelay: number, portData: any) {
